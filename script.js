...
function fullDate() {
  const time = new Date();
  longDate.textContent = `Сегодня ${day[time.getDay() - 1]} ${twoDigits(
    time.getDate()
  )} ${month[time.getMonth()]} ${time.getFullYear()} года 
  ${twoDigits(time.getHours())} ${hour} ${twoDigits(
    time.getMinutes()
  )} ${minute} ${twoDigits(time.getSeconds())} ${second}`;
}
...
function shortDate() {
  const time = new Date();
  shortTextDate.textContent = `${twoDigits(time.getDate())}.${twoDigits(
    time.getMonth() + 1
  )}.${time.getFullYear()} - 
  ${twoDigits(time.getHours())}:${twoDigits(time.getMinutes())}:${twoDigits(
    time.getSeconds()
  )}`;
}
shortDate();
